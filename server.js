...
// - GET  /journal                     <- raw JSON of journal entries (Journal tab)
// - PUT  /journal/:date                <- upsert one day's journal entry (date = YYYY-MM-DD)
// - DELETE /journal/:date              <- delete one day's journal entry
//
...
app.put('/journal/:date', (req, res) => {
  ...
  const entry = {
    ...
    premarket: typeof body.premarket === 'string' ? body.premarket : '',
    losses: typeof body.losses === 'string' ? body.losses : '',
    issues: typeof body.issues === 'string' ? body.issues : '',
    wins: typeof body.wins === 'string' ? body.wins : '',
